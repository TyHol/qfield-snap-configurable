...
Item {
  ...
  Settings {
    ...
    // When false, skips the plugin camera and opens the feature form directly,
    // letting the user take the photo through the form's native camera widget
    property bool openCameraImmediately: true
  }
  ...
  QfToolButton {
    ...
    onClicked: {
      ...
      if (appSettings.openCameraImmediately)
        cameraLoader.active = true
      else
        snapToForm()  // skip plugin camera; user takes photo via the form widget
    }
  ...
  Dialog {
    ...
    Column {
      ...
      // Camera mode toggle — disable if the built-in camera does not work on this device
      Row {
        width: parent.width
        spacing: 8
        Switch {
          id: cameraImmediateSwitch
          checked: appSettings.openCameraImmediately
        }
        Label {
          text: cameraImmediateSwitch.checked
            ? qsTr("Open camera immediately on tap")
            : qsTr("Open feature form — take photo from within the form")
          wrapMode: Text.WordWrap
          width: parent.width - cameraImmediateSwitch.width - 8
          anchors.verticalCenter: cameraImmediateSwitch.verticalCenter
          font.pixelSize: 11
        }
      }
    ...
    onAccepted: {
      ...
      appSettings.openCameraImmediately = cameraImmediateSwitch.checked
    }
  }

  // Builds a GPS-positioned blank feature and opens the feature form without
  // pre-opening the camera — the user takes the photo from within the form using
  // the device's native camera widget. Used when openCameraImmediately is false.
  function snapToForm() {
    var layer = resolveLayer()
  ...
  function snap(path) {
    ...
    let relativePath = 'DCIM/' + today.getFullYear()
                               ...
                               + ("00" + String(today.getHours())).slice(-2)
                               + ("00" + String(today.getMinutes())).slice(-2)
                               + ("00" + String(today.getSeconds())).slice(-2)
